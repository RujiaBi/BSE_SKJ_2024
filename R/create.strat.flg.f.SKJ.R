...
create.strat.flg.f.SKJ = function(lat.5deg,lon.5deg,is.lwrght,month,setype,vessel.class,PS) {
  ...
  if(PS=="OBJ")  {
    area<-rep(1,nrecs)
    area[lon.5deg>=(-118) & lon.5deg<=(-97.5) & lat.5deg<=(-7.5)]<-2
    area[lon.5deg>=(-92.5) & lat.5deg<=(-17.5)]<-3
    area[lon.5deg>=(-92.5) & lat.5deg>=(-12.5) & lat.5deg<=(-7.5)]<-4
    area[lon.5deg>=(-118) & lat.5deg>=(-2.5)]<-5
    
    print("Using fishery stratification: regression tree in 2024 for OBJ")
  ...
  if(PS=="DEL") {
    ...
    print("Using fishery stratification: regression tree in 2024 for DEL")
  ...
  if(PS=="NOA") {
    area<-rep(1,nrecs)
    area[lon.5deg>=(-97.5) & lon.5deg<=(-87.5) & lat.5deg<=(12.5)]<-2
    area[lon.5deg>=(-82.5) & lat.5deg<=(-2.5)]<-3
    area[lon.5deg>=(-82.5) & lat.5deg>=(2.5)]<-4
    area[lon.5deg>=(-118) & lat.5deg>=(17.5)]<-5
    
    print("Using fishery stratification: regrssion tree in 2024 for UNA")
